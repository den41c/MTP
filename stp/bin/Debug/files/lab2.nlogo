...
to setup-nodes
  ...
  [
    ...
    set own-ticks 0
    setup-strategy
    set previous-strategy strategy-number
    ifelse i < playersCount
    [
      set strategies array:from-list n-values redDelay [(random strategy-count)]
      set color red
      set delay redDelay
    ]
    [
      set strategies array:from-list n-values blueDelay [(random strategy-count)]
      set color blue
      set delay blueDelay
    ]
    set i i + 1
...
to setup-strategy
  set strategy-number (random strategy-count)
end
to clear-turtle
  set ispaired? false
  set prize 0
end
to setup-game-matrix
  set blue-matrix matrix:from-row-list list ( list blue1r1 blue1r2 ) ( list blue2r1 blue2r2)
  set red-matrix matrix:from-row-list list (list reds1b1 reds2b1) (list reds1b2 reds2b2)
end
to go
  clear-links
  ask turtles
  ...
  ask turtles
  [
    set label strategy-number
    set own-ticks own-ticks + 1
    if own-ticks = delay - 1 [set own-ticks 0]
    if own-ticks = delay [set own-ticks 0]
  ]
  set-strategy-as-future-strategy
  tick
end
...
to play
  ...
  let turtle-strategy strategy-number
  let choice one-of (turtles in-radius radius with [not ispaired? and color != turtle-color])
  if choice != nobody [
    set ispaired? true
    let opponent-strategy 0
    ask choice [
      set ispaired? true
      set opponent-strategy strategy-number
      let opponent-color color
    ]
    create-link-with choice
    let coordinates list 0 0
    let m1 matrix:from-row-list [[0 0] [0 0]]
    let m2 matrix:from-row-list [[0 0] [0 0]]
    ifelse turtle-color = blue
    [
      set coordinates list turtle-strategy opponent-strategy
      set m1 blue-matrix
      set m2 red-matrix
    ]
    [
    ...
    set prize (matrix:get m1 (item 0 coordinates) (item 1 coordinates))
    ask choice[
      set prize (matrix:get m2 (item 0 coordinates) (item 1 coordinates))
    ]
  ]
end
to save-previous-strategy
  ask turtles with [ispaired?]
  [
    set previous-strategy strategy-number
  ]
end
to change-strategy
    ask turtles with [ispaired? and color = blue]
    [
      let turtle-strategy strategy-number
      let opponent-strategy 0
      ask link-neighbors
      [
        set opponent-strategy previous-strategy
      ]
      let column matrix:get-column blue-matrix opponent-strategy
    set-future-strategy((position max(column) column))
    ]

    ask turtles with [ispaired? and color = red]
    [
      let turtle-strategy strategy-number
      let opponent-strategy 0
      ask link-neighbors
      ...
      let row matrix:get-row red-matrix opponent-strategy
      set-future-strategy( (position max(row) row))
    ]
end
to set-future-strategy [s]
  array:set strategies own-ticks s
end
to set-strategy-as-future-strategy
  ask turtles
  [
    set strategy-number array:item strategies own-ticks
  ]
end
@#$#@#$#@
GRAPHICS-WINDOW
...
"" ""
PENS
"reds" 1.0 0 -2674135 true "" "let y-active-turtles turtles with [ispaired? and color = red]\nlet res 0\nif count y-active-turtles != 0\n[\n  set res (sum [prize] of y-active-turtles) / (count y-active-turtles)\n]\nplot res"
"blues" 1.0 0 -13345367 true "" "let active-turtles turtles with [ispaired? and color = blue]\nlet res 0\nif count active-turtles != 0\n[\n  set res (sum [prize] of active-turtles) / (count active-turtles)\n]\nplot res"
